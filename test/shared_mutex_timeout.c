...
main(void)
  {
    ...
    r = _MCF_shared_mutex_lock_shared(&mutex, (const int64_t[]){ (int64_t) _MCF_hires_utc_now() + 1101 });  /* absolute  */
    assert(r == 0);
    ...
    now = _MCF_perf_counter();
    r = _MCF_shared_mutex_lock_exclusive(&mutex, (const int64_t[]){ (int64_t) _MCF_hires_utc_now() + 1101 });  /* absolute  */
    assert(r == -1);
    delta = _MCF_perf_counter() - now;
    fprintf(stderr, "delta = %.6f\n", delta);
    assert(delta >= 1100);
