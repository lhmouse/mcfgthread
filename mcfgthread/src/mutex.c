  ...
  try_lock_loop:
    ...
    for(;;)
      if(old.__locked == 0) {
        ...
        sp_budget = (__MCF_MUTEX_SP_NFAIL_THRESHOLD - old.__sp_nfail) * 16;
        new.__locked = 1;
    ...
    NTSTATUS status = NtWaitForKeyedEvent(NULL, mtx, false, &(nt_timeout.li));
    while(status != STATUS_WAIT_0) {
      ...
      status = NtWaitForKeyedEvent(NULL, mtx, false, __MCF_NT_TIMEOUT_0);
    }
