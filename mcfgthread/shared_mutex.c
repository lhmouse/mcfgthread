...
_MCF_mutex_lock_slow(_MCF_mutex* mutex, const int64_t* timeout_opt)
  {
     ...
     * is less than `__MCF_SHARED_MUTEX_SP_MASK_M` and `__sp_nfail` is less than
     * `__MCF_SHARED_MUTEX_SP_NFAIL_THRESHOLD`, which means the current thread is
  ...
  try_lock_loop:
    ...
    do {
      new = old;
      new.__locked = 1;
      spinnable = (uint32_t) ((old.__sp_mask - __MCF_SHARED_MUTEX_SP_MASK_M) & (old.__sp_nfail - __MCF_SHARED_MUTEX_SP_NFAIL_THRESHOLD)) >> 31;
      new.__sp_mask = (old.__sp_mask | (old.__sp_mask + (old.__locked & spinnable))) & __MCF_SHARED_MUTEX_SP_MASK_M;
      new.__sp_nfail = do_adjust_sp_nfail(old.__sp_nfail, (int) old.__locked * 2 - 1) & __MCF_SHARED_MUTEX_SP_NFAIL_M;
      new.__nsleep = (old.__nsleep + (old.__locked & ~spinnable)) & __MCF_SHARED_MUTEX_NSLEEP_M;
    }
    ...
    if(my_mask != 0) {
      ...
      register int spin = (int) (__MCF_SHARED_MUTEX_SP_NFAIL_THRESHOLD - old.__sp_nfail);
      __MCF_ASSERT(spin > 0);
      spin *= (int) (__MCF_SHARED_MUTEX_MAX_SPIN_COUNT / __MCF_SHARED_MUTEX_SP_NFAIL_THRESHOLD);
      ...
      while(__builtin_expect(spin, 100) > 0) {
        ...
        do {
          ...
          new = old;
          new.__locked = 1;
          new.__sp_nfail = do_adjust_sp_nfail(old.__sp_nfail, -1) & __MCF_SHARED_MUTEX_SP_NFAIL_M;
        }
      ...
      do {
        new = old;
        new.__locked = 1;
        new.__sp_nfail = do_adjust_sp_nfail(old.__sp_nfail, (int) old.__locked - 1) & __MCF_SHARED_MUTEX_SP_NFAIL_M;
        new.__nsleep = (old.__nsleep + old.__locked) & __MCF_SHARED_MUTEX_NSLEEP_M;
      }
      while(!_MCF_atomic_cmpxchg_weak_pptr_acq(mutex, &old, &new));
    ...
    while(err != 0) {
      ...
      do {
        ...
        new = old;
        new.__nsleep = (old.__nsleep - 1U) & __MCF_SHARED_MUTEX_NSLEEP_M;
      }
...
_MCF_mutex_unlock_slow(_MCF_mutex* mutex)
  {
    ...
    do {
      new = old;
      new.__locked = 0;
      new.__sp_mask = (old.__sp_mask & (old.__sp_mask - 1U)) & __MCF_SHARED_MUTEX_SP_MASK_M;
      wake_one = _MCF_minz(old.__nsleep, 1);
      new.__nsleep = (old.__nsleep - wake_one) & __MCF_SHARED_MUTEX_NSLEEP_M;
    }
