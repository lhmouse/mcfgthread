...
#define __MCF_DECLSPEC_ONCE_IMPORT  __MCF_DLLEXPORT
#define __MCF_DECLSPEC_ONCE_INLINE  __MCF_DLLEXPORT
#include "once.h"
#include "xglobals.i"
...
_MCF_once_wait_slow(_MCF_once* once, const int64_t* timeout_opt)
  ...
  try_lock_loop:
    ...
    while(status != STATUS_WAIT_0) {
      ...
      status = __MCF_keyed_event_signal(once, (LARGE_INTEGER[]) { 0 });
    }
