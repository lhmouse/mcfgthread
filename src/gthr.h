...
__MCF_gthr_recursive_mutex_lock(__gthread_recursive_mutex_t* __rmtx) __MCF_NOEXCEPT
  {
    ...
    if(__MCF_ATOMIC_LOAD_N_RLX(&(__rmtx->__owner)) != __my_tid) {
      ...
      __MCF_ATOMIC_STORE_N_RLX(&(__rmtx->__owner), __my_tid);
...
__MCF_gthr_recursive_mutex_unlock(__gthread_recursive_mutex_t* __rmtx) __MCF_NOEXCEPT
  {
    uint32_t __my_tid = _MCF_thread_self_tid();
    __MCFGTHREAD_CHECK(__MCF_ATOMIC_LOAD_N_RLX(&(__rmtx->__owner)) == __my_tid);
    ...
    if(__rmtx->__depth == 0) {
      ...
      __MCF_ATOMIC_STORE_N_RLX(&(__rmtx->__owner), 0);
